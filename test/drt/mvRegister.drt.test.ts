...
describe('DRT: MV-Register via SQL script pipeline', () => {
  ...
  drt
    .prop([arbMvSqlCase], { numRuns: drtRuns })
    ('MV-focused SQL scripts match Lean and TypeScript', async (input) => {
      const sqlStatements = input.values.map(
        (value) => `UPDATE tasks SET status = ${renderLiteral(value)} WHERE id = 'task-1'`,
      );
      sqlStatements.push("SELECT status FROM tasks WHERE id = 'task-1'");
